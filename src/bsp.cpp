...
BSP::BSP(std::filesystem::path relativePath) {
    const bool LOG_HEADER_INFO = false;
    ...
    if (LOG_HEADER_INFO) {
        ...
        for (int i = 0; i < 15; i++) {
            std::cout << "lump " << i << " has length "
                      << this->header.lump[i].nLength << "\n";
    ...
    file.read(this->entities,
              this->header.lump[LUMP_ENTITIES].nLength * sizeof(char));

    // Textures
    Lump textureLumpHeader = this->header.lump[LUMP_TEXTURES];
    file.seekg(textureLumpHeader.nOffset);
    file.read(reinterpret_cast<char *>(&this->textureHeader),
              sizeof(this->textureHeader));

    this->textureOffsets =
        std::vector<uint32_t>(this->textureHeader.nMipTextures);
    file.read(reinterpret_cast<char *>(this->textureOffsets.data()),
              sizeof(uint32_t) * this->textureHeader.nMipTextures);
    ...
    if (LOG_TEXTURES) {
        std::cout << "Amount of textures in map: "
                  << this->textureHeader.nMipTextures << "\n";
    ...
    if (LOG_TEXINFO) {
        ...
        for (const auto &texinfo : this->texInfo) {
            std::cout << "Texinfo vS: " << texinfo.vS.x << " " << texinfo.vS.y << " "
                      << texinfo.vS.z << "\n";
            std::cout << "Texinfo vT: " << texinfo.vT.x << " " << texinfo.vT.y << " "
                      << texinfo.vT.z << "\n";
    ...
    if (LOG_VERTICES) {
        ...
        if (numVertices > 0) {
            std::cout << "First vertex: " << this->vertices[0].x << ", "
    ...
    if (LOG_FACES) {
        ...
        if (numFaces > 0) {
            std::cout << "First face has " << this->faces[0].nEdges
                      << " edges, iFirstEdge=" << this->faces[0].iFirstEdge << "\n";
        }
    }
    ...
    if (LOG_EDGES) {
        ...
        for (const Edge &edge : this->edges) {
            std::cout << "Read edge with vertices " << edge.iVertex[0] << " and "
                      << edge.iVertex[1] << "\n";
        }
    }
...
glm::vec2 BSP::GetTextureCoords(glm::vec3 vertexPosition, Face face) const {
    ...
    return {nonnormalTexCoords.x / mip.nWidth,
        nonnormalTexCoords.y / mip.nHeight};
}

glm::vec2 BSP::GetNonNormalizedTextureCoords(glm::vec3 vertexPosition,
                                             Face face) const {
    Textureinfo hTexInfo = texInfo[face.iTextureInfo];
    MipTex mip = textures[hTexInfo.iMiptex];
...
glm::vec2 BSP::GetLightmapCoords(glm::vec3 vertexPosition, Face face) const {
    ...
    auto lightmapCoords =
        glm::vec2{lightmap_u / lightmapWidth, lightmap_v / lightmapHeight};
